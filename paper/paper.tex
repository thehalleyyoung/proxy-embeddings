...
this?" into a budget question with a curve behind it. The fall is a
property of the \emph{kind} of target rather than of rarity as such:
targets a generator can write into the artifact stay near 100\% at every
rarity, and only targets that must emerge from its behaviour decay.
...
{\footnotesize
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.3165\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.2723\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.2040\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.2072\linewidth-2\tabcolsep\relax}@{}}
\toprule\noalign{}
...
{\footnotesize
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{\dimexpr 0.4101\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.2900\linewidth-2\tabcolsep\relax}>{\raggedright\arraybackslash}p{\dimexpr 0.2999\linewidth-2\tabcolsep\relax}@{}}
\toprule\noalign{}
rarity of the target & programs: blind / decoy / \textbf{instruct} &
expressions: blind / decoy / \textbf{instruct} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{never produced} & 0.0\% / 0.0\% / \textbf{25.0\%} & 0.0\% /
2.8\% / \textbf{94.4\%} \\
{[}0.001, 0.02) & 2.8\% / 2.8\% / \textbf{30.6\%} & 0.0\% / 8.3\% /
\textbf{100.0\%} \\
{[}0.02, 0.10) & 5.6\% / 2.8\% / \textbf{55.6\%} & 8.3\% / 0.0\% /
\textbf{100.0\%} \\
{[}0.10, 0.30) & 5.6\% / 5.6\% / \textbf{63.9\%} & 0.0\% / 0.0\% /
\textbf{100.0\%} \\
{[}0.30, 1.01) & 58.3\% / 16.7\% / \textbf{100.0\%} & 33.3\% / 0.0\% /
\textbf{100.0\%} \\
\end{longtable}}
...
unconditioned prompt, both controls score zero on programs and
instruction reaches a quarter of them; on expressions instruction
reaches 94.4\% where blind prompting reaches none. A pipeline restricted
to sampling and filtering cannot obtain these artifacts at any budget,
because the rate it is filtering is zero. Asking obtains them.

\subsubsection{3.3 What governs how far it
reaches}\label{33-what-governs-how-far-it-reaches}

The two decoders disagree by a factor of four in the tail and the reason
is not rarity. It is what kind of thing the target is.

A regular expression required to match \texttt{2024-01-31} can be
\emph{written} to match it: the requirement decomposes into the syntax
the generator is already producing, so compliance is near-total
everywhere, and the natural-corpus rarity of the target is nearly
irrelevant. A program required to return exactly 169 on a given
thirteen-element list cannot be written that way. The value has to
\emph{come out} of whatever the function computes, and the generator has
to find a computation that both is a plausible general-purpose function
and lands on that number. That is a search, not a transcription, and it
fails most of the time in the tail.

So the useful distinction for a practitioner is not rare against common
but \textbf{constructible against emergent}:

\begin{itemize}
\tightlist
\item
  A \textbf{constructible} target is one the generator can satisfy by
  writing something specific into the artifact. Compliance is high and
  roughly flat in rarity. Aim freely.
\item
  An \textbf{emergent} target is one that falls out of the
  artifact\textquotesingle s behaviour rather than being written into
  it. Compliance falls with rarity --- 100\% to 25\% here --- and the
  budget per target has to rise accordingly.
\end{itemize}

Rarity in the natural corpus is a good predictor \emph{within} an
emergent domain and a poor one across domains, which is worth knowing
before planning a budget from one domain\textquotesingle s curve. The
generalization to the closed-weight setting is direct: "this shot must
contain a red door on the left" is constructible; "this shot must have
the pacing of a specific edit rhythm" is emergent, and the second needs
the budget the curve implies.
