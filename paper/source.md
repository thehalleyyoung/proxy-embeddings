...
90.7%), and relaxing the acceptance criterion *after* generation recovers almost
nothing (92.2% against 90.7%), because failures on emergent targets are wholesale
rather than near-misses. Observability, not tolerance, is the axis that moves
...
### 3.4 Ask for a range: a small real effect, and a large artifact we removed

The first version of this experiment reported that stating an emergent target as
a band rather than a point was worth 18.6 points, and 51.5 points in the rarest
band. That was mostly not a tolerance effect. It is worth walking through,
because the error is one a reader could easily repeat and the instrument that
caught it is now in the tool.

**What went wrong.** The two ask types did not complete at the same rate. At a
900-token generation budget the exact ask returned a usable program 68.2% of the
time against the band ask's 87.6% — a 19.4-point spread — and a generation that
returns nothing was scored as a failure. Attrition correlated with the treatment
manufactures an effect on its own. Raising the budget to 2,600 tokens with one
retry equalizes the arms at 94.6% and 99.2%, a 4.7-point spread, and the whole
compliance level moves: the exact ask goes from 66.7% to **90.7%**. Most of what
the first run measured was token starvation, which looks exactly like task
difficulty from the outside.

**The result at equal completion**, 48 targets × 3 seeds, same decoder, same
generator, one difference in the ask:

| asked | scored | per call issued | of those that answered |
|---|---|---|---|
| exactly | exactly | 90.7% | 95.9% [91.8, 99.2] |
| **as a band** | **on the band** | **97.7%** | **98.4%** [96.1, 100.0] |
| as a band | exactly | 36.4% | 36.7% [28.1, 45.3] |
| exactly | on the band | 92.2% | 97.5% [94.3, 100.0] |

The tolerance benefit is **+7.0 points per call issued** and **+2.5 points among
generations that answered**, with overlapping intervals on the second. So a band
helps, modestly, and we cannot separate how much of even that is a residual
return-rate effect from how much is accuracy. The honest statement is that it is
worth about seven points of yield and is not established as making the artifact
more likely to be right.

**Both cross-terms survive, and they carry the prescription.** These compare two
*scorings* of one set of generations rather than two arms, so attrition cannot
reach them.

*A band is not free.* Asking for a band and then insisting on the exact value
gives 36.4%, far below asking exactly (90.7%). The generator spends the slack it
is given. If the point value genuinely matters, asking for a range around it is
much worse than asking for the point.

*Failures are not near-misses.* Asking exactly and scoring on a ±10% band gives
92.2% against 90.7% scored exactly — 1.5 points. When an exact ask fails on an
emergent target it fails wholesale rather than landing nearby. So the tolerance
...
line. §3.7 shows the opposite profile is what an unobservable criterion looks
like, and §3.8 separates the two mechanisms.

**The general lesson is not about tolerance.** It is that a generation budget too
small for the harder arm is indistinguishable, in the output, from the harder arm
being harder. Report completion rates by arm, always, and treat a spread as a
reason to equalize before interpreting anything. `decodergap.completion_check`
refuses a cross-arm verdict above a 15-point spread; it fires on four of the six
runs in this paper as originally executed.
...
### 5.2 The oracle is an oracle for the metric you chose

The advice to decode and select in the artifact's space assumes you know which
artifact-space quantity you want, and that assumption deserves stating because it
can fail.

A companion measurement fit the saturation exponent of eight generation policies
on roughly 19,200 real generations — the rate at which the minimum gap to a
held-out block decays with corpus size, which estimates the dimension of what a
policy can reach. The power law fits at *R*² between 0.992 and 0.998, and the
fitted dimension runs from 3.65 [3.13, 4.38] for naive prompting to 14.60 [13.56,
15.81] for a composed policy, with the naive interval disjoint from every
conditioned policy's. So conditioning moves the exponent and not merely the
constant.

The relevant result for this paper is that **three artifact-space orderings of
the same eight policies disagree**. Ranked by reach, the composed policy is first
and naive last. Ranked by coverage of a held-out human-written reference, a mined
persona catalogue is first. Ranked by the quality of a model fine-tuned on the
corpus, the two persona pipelines place sixth and eighth of eight. A policy can
enlarge its support and spend the enlargement where no downstream task wants it.

This is the same lesson as §4 arriving from the other side, and it bounds our own
prescription. The oracle of §5.1 is greedy marginal coverage of one specific
coverage functional; it is an upper bound on that functional and not on anything
else. Choosing the wrong artifact-space quantity is a cheaper mistake than
measuring in the wrong space entirely — at least it is measured on the artifact —
but it is a mistake the cost comparison does not protect you from. Decide what
you want the artifact to do before deciding to decode, and if reach, coverage and
downstream utility might disagree for your task, measure the one you are actually
buying.
...
**`completion_check`** refuses a cross-arm verdict when the arms did not answer
at equal rates. This is in the tool because it caught a 19-point effect of ours
that was not there (§3.4) and a boundary result of a collaborator's that rested
on five observations out of sixty. In both cases the tell was the sample count
rather than the effect size, and in both cases the spurious result was cleaner
and more interesting than the truth.
...
Six failures now, and the sixth is a different species. The tolerance main
effect (§3.4) was real as an intent-to-treat number and not real as an accuracy
claim: the exact ask returned nothing 32% of the time against the band ask's 12%,
and conditional on the generator answering at all the two are indistinguishable at
97.7% and 97.3%. That is not a correlation-shape error. It is attrition
correlated with the treatment, which is the classic way an experiment
manufactures an effect, and we found it only because a collaborating session hit
the same failure and suggested checking completion rates by arm.

Five failures of one kind, one survivor class, The failures were not bad luck: correlation
