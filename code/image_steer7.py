...
-- THIS FILE: pre-expansion, axes fixed from item 0 -------------------------
Mid-run expansion adds an axis at the round it fires, so the axis conditions
only the items generated after it. Measured on the two expansion arms: axes
introduced in the first round reach 80% of the corpus, the second round reaches
43%, and in one arm the final pair entered at item 56 and conditioned 4 of 60
items -- 7%. An axis present for 7% of a run cannot move a corpus-level score
whatever its merit, so the expansion arms never tested the perceptual axes at
full strength.

This arm separates the two questions. The judge's proposals are already known,
so they are seeded alongside the elicited art-theory axes and are in force from
item 0. No mid-run expansion. The contrast against the steer2 baseline is then
"the same budget with four perceptual axes throughout" rather than "with them
for part of the run".


-- THIS FILE: subset conditioning ------------------------------------------
Obedience falls as the number of contracts in one prompt rises. Measured across
four arms: 7.0 contracts per item gives 0.417 of renders sharing a palette with
a nearest neighbour, 11.0 gives 0.667, and mean per-axis realization halves from
0.0219 to 0.0126 between 8.7 and 11.0. An axis introduced late is diluted by the
items before it; an axis introduced early dilutes every other contract. Both
losses come from the same place -- one prompt carrying N contracts, with
compliance falling in N.

Nothing requires every axis to appear in every spec. `sample_spec` assigns a
level to each applicable axis because that is the natural reading of "a point in
the lattice", but the lattice is a description of what the corpus can reach, not
a description of what one prompt must carry. Conditioning on a SUBSET per item
decouples the two: the full axis set stays available across the corpus while any
single prompt stays inside the contract budget.

The subset is drawn least-used-axis-first, so every axis still accumulates
roughly equal usage across the run -- the same balancing argument that fixed
level starvation, applied one level up, to axes rather than levels.
"""
...
GAP_MAX_AXES = 0         # no mid-run expansion: the axes are pre-seeded
CONTRACTS_PER_ITEM = 7   # the load at which compliance was highest
...
PRESEEDED_PERCEPT = [{'name': '[percept] Camera viewpoint and angle', 'levels': ['Extreme low-angle view looking sharply upward from ground level', 'High-angle view looking down from above the scene', 'True overhead bird’s-eye view with surfaces seen planimetrically', 'Strict side-profile view with the subject seen edge-on', 'Strong Dutch-angle three-quarter view with the horizon visibly tilted', 'Centered eye-level frontal view']}, {'name': '[percept] Shot scale', 'levels': ['Extreme macro detail showing only a small surface fragment', 'Tight close-up filling the frame with one object or face', 'Medium view showing a complete object or small arrangement', 'Wide view showing the arrangement within a substantial environment', 'Distant long shot with tiny subjects surrounded by expansive space']}, {'name': '[percept] Motion and temporal blur', 'levels': ['Pin-sharp freeze-frame with every edge clearly resolved', 'Directional motion blur on moving forms while the background remains sharp', 'Panning exposure with a sharp moving subject against horizontally streaked surroundings', 'Long-exposure trails and smeared highlights from movement through the frame', 'Multiple-exposure ghosting showing several displaced positions of the same forms', 'Radial zoom blur converging toward the image center']}, {'name': '[percept] Lighting direction and hardness', 'levels': ['Flat overcast illumination with nearly shadowless, even tonal coverage', 'Hard overhead midday light with short, sharply defined shadows', 'Strong side light producing deep chiaroscuro and a narrow bright key', 'Low-angle warm sunset light with long shadows and golden highlights', 'Backlight creating a bright rim and mostly silhouetted forms', 'Colored theatrical light with contrasting cool and warm directional sources']}]
...
def load_axes(tree: AxisTree) -> AxisTree:
    ...
    if not tree.axes and src.exists():
        ...
        for a in PRESEEDED_PERCEPT:
            if not any(x.name == a["name"] for x in tree.axes):
...
def run(objective: str, target: int = TARGET, tag: str = "steer7"):
    ...
    axis_use: dict = {}       # axis name -> times it has appeared in a spec

    def subset_spec(rng_: random.Random) -> dict:
        """One spec over a bounded subset of axes, least-used first."""
        full = tree.sample_spec(rng_)
        if len(full) <= CONTRACTS_PER_ITEM:
            for k in full:
                axis_use[k] = axis_use.get(k, 0) + 1
            return full
        order = sorted(full, key=lambda k: (axis_use.get(k, 0), rng_.random()))
        keep = order[:CONTRACTS_PER_ITEM]
        for k in keep:
            axis_use[k] = axis_use.get(k, 0) + 1
    ...
    while len(recs) < target:
        ...
        for _ in range(n_slots):
            pool = [subset_spec(rng) for _ in range(POOL)]
    ...
    summary = {
        ...
        "contracts_per_item": CONTRACTS_PER_ITEM,
        "axis_usage": dict(sorted(axis_use.items(), key=lambda kv: -kv[1])),
...
if __name__ == "__main__":
    ...
    run(obj, int(sys.argv[2]) if len(sys.argv) > 2 else TARGET,
        sys.argv[3] if len(sys.argv) > 3 else "steer7")
