...
-- THIS FILE: selection breadth ---------------------------------------------
steer4 measured the improvement its own mechanism was built for and did not
move the corpus metric: the perceptual axes gap expansion adds are 2.3x better
realized than the elicited ones, and CLIP Vendi still fell 43.8 -> 42.4 against
the baseline. Realization and diversity are not the same quantity.

The hypothesis this file tests is that the binding constraint is SELECTION, not
conditioning. Every arm so far drew POOL = 6 candidate prompts per slot and
kept the max-min pick, so how far the corpus spreads is set by the breadth of
that pool, not by the richness of the lattice the pool is drawn from. A better
lattice proposes better candidates and cannot help if only six are considered.

Pool candidates are text-only -- exactly one of them is ever rendered -- so
raising POOL costs chat calls and no renders at all. Everything else is
identical to steer4.
"""
...
TARGET = 60
WORKERS = 6
POOL = 16          # was 6: the quantity this arm tests
QUALITY = "high"
MINE_EVERY = 20
LEDGER_SLICE = 8
LEARN_AFTER = 14
...
def run(objective: str, target: int = TARGET, tag: str = "steer5"):
...
if __name__ == "__main__":
    ...
    run(obj, int(sys.argv[2]) if len(sys.argv) > 2 else TARGET,
        sys.argv[3] if len(sys.argv) > 3 else "steer5")
